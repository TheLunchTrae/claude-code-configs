...
| `sync-configs.ts` | Implements `/sync-configs`. Registers `sync_configs_plan` (fetch upstream manifest, classify each tracked path) and `sync_configs_apply` (write decisions, persist version on full success). Last-synced version stored at `~/.opencode-data/sync-configs/<project>/version`. |
