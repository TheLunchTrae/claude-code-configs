...
- **`sync_configs_plan`** — read-only. Fetches the manifest from `raw.githubusercontent.com/thelunchtrae/claude-code-configs/main/opencode/.opencode/sync-configs-manifest.json`, `JSON.parse`s it, flattens `paths` (an object whose values are string arrays per category) into a single sync list, reads the local last-synced version, and fetches every tracked path in parallel via global `fetch()` (Node 18+; no curl). Each path is classified: `unchanged` (identical to upstream), `updated` (local missing, or every non-empty local line also appears in upstream so applying remote loses nothing the user added), `needs-user-decision` (local has lines not present upstream — drift), or `failed` (HTTP non-2xx, empty body, or HTML 404 sentinel). `opencode.jsonc` is hard-coded as always-defer when content differs (the `permission` block requires manual judgment per `opencode/.claude/CLAUDE.md`). Returns `short_circuit` when manifest version equals local version. Writes nothing.
- **`sync_configs_apply`** — mutating. Takes `manifest_version`, `paths_to_update` (the `updated` entries with their remote content), `decisions` (one per resolved `needs-user-decision` path: `preserve` / `drop` / `custom` + content), `unresolved` (skipped paths), `delete_paths`, and `failed` from the plan. Writes `paths_to_update` and applied decisions atomically (temp file + rename via `atomicReplace`). Deletes `delete_paths` only when the plan returned them (first-run safety: plan returns empty `delete_paths` when no prior version was found). Persists `manifest_version` to `~/.opencode-data/sync-configs/<project>/version` only when nothing failed and `unresolved` is empty — partial-failure runs leave the file untouched so the next `/sync-configs` re-enters and retries.
