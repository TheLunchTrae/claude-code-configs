...
2. **Reviewer** — send the design for review; if issues are raised, address them and loop back; if none, proceed
3. **Developer** — send the approved design for implementation; if a blocker is reported, resolve it and return to step 2
4. **Reviewer** — send the implementation for review; if issues are raised, send them to the developer and re-run; if none, report completion
