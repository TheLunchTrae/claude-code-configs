...
Procedure: (1) call `sync_configs_get_manifest` → returns `{version, paths, deleted, last_local_version, short_circuit}`; on `short_circuit` the command exits with `Already up to date (version <N>).`. (2) Determine the configs root from cwd or `AGENTS.md`. (3) Walk every path in the manifest in order: `Read` local, call `sync_configs_fetch_file({path})` for upstream, call `sync_configs_classify({path, local, remote})` for the action verdict (`unchanged` / `updated` / `needs-user-decision` with `local_only_lines` and `reason`), then `Write` upstream verbatim for `updated`, surface drift to the user for `needs-user-decision` (verbatim quote of `local_only_lines`, recommendation **drop** by default; choices: drop / preserve / custom / skip), or no-op for `unchanged`. Local-missing skips classify and writes upstream directly. (4) Apply deletions from `manifest.deleted` only when `last_local_version` was defined (first-run safety). (5) Call `sync_configs_record_version({version})` only when `failed[]` and `unresolved[]` are both empty — partial-failure runs leave the version state alone so the next `/sync-configs` retries. (6) Print the structured report (Updated / Unchanged / Preserved / Deleted / Failed / Unresolved with footers for non-advanced version, first-run skipped deletions, and plugin self-update).
