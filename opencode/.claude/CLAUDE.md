...
Fetches the manifest at `opencode/.opencode/sync-configs-manifest.md` from `raw.githubusercontent.com/thelunchtrae/claude-code-configs/main/` on every run (the local manifest is never consulted). Reads the `Version:` integer from the manifest and compares it against `~/.opencode-artifacts/_global/sync-configs/last-version.txt`; if they match, the command short-circuits with `Already up to date (version <N>).` and fetches nothing further. On version mismatch, for each path in the manifest it fetches the remote copy, compares it to the local copy under `~/.claude/`, and merges changes (prefer remote, preserve clear local-only customizations, ask user when intent is ambiguous or diffs are large). Paths under the manifest's `## Deleted` section are removed from the local tree — but **only when the state file already holds a prior version**; on first-run (no state file), deletions are skipped with a note so unrelated local files are never removed. After the sync attempt, the state file is written with the new version unconditionally (even on partial fetch failures). If the manifest fetch itself fails, the command aborts immediately and touches no local files. Reports updated, unchanged, deleted, and failed files.
