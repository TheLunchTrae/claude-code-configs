...
Implemented as a hybrid plugin tool + thin slash command: `plugins/sync-configs.ts` does the mechanical work (manifest fetch, version compare, parallel path fetch, classification, atomic version persist), and the slash command is a wrapper that calls `sync_configs_plan` → surfaces any `needs-user-decision` cases to the user (with the `local_only_lines` quoted verbatim and the recommendation defaulting to **drop**) → collects preserve / drop / custom choices → calls `sync_configs_apply` with the decisions → prints the structured report. The plan tool short-circuits with `Already up to date (version <N>).` when the local version (read from `~/.opencode-data/sync-configs/<project>/version`) matches the manifest. On version mismatch the plan classifies each path as `unchanged`, `updated` (write remote verbatim — covers local-missing and the case where every non-empty local line also appears in the remote, so applying upstream loses nothing the user added), `needs-user-decision` (local has drift lines not present upstream, or path is `opencode.jsonc` which always defers), or `failed` (HTTP non-2xx, empty body, HTML 404 sentinel). The apply tool writes files atomically, deletes `delete_paths` (which the plan returns empty on first-run for safety), and persists the version only when nothing failed and every needs-user-decision case was resolved — partial-failure runs leave the version untouched so the next `/sync-configs` re-enters and retries. When `plugins/sync-configs.ts` was among the updated paths, the report appends a restart-and-`bun install` reminder. First-time setup (curl the plugin files from upstream) is documented in `opencode/.opencode/README.md`.
...
### `plugins/sync-configs.ts` — SyncConfigsPlugin
...
- **Storage** — `~/.opencode-data/sync-configs/<project>/version` contains just the integer (no schema, no format). `SYNC_CONFIGS_ROOT` is exported from `lib/project.ts` next to `ARTIFACT_ROOT` and `MEMORY_ROOT`. Removing the version file forces the next run to re-sync every tracked file from scratch, but deletions from the manifest's `## Deleted` section will be skipped on that run since the plugin cannot prove those files came from a previous sync.
- **First-run bootstrap** — `/sync-configs` is plugin-only; there is no inline curl fallback. Brand-new users (not cloning the repo) bootstrap the plugin via the shell snippet documented under "First-time setup" in `opencode/.opencode/README.md`, which curls `plugins/sync-configs.ts`, `plugins/lib/project.ts`, and `tsconfig.json` from upstream and runs `bun install`. After restart, `/sync-configs` works.
- **Plugin self-update** — `plugins/sync-configs.ts` is in the manifest. Plugins load at session start into closed-over JS objects in memory; overwriting the `.ts` file mid-run does not affect the running tool, only the next session. The apply tool detects when its own file was among the updated paths and surfaces a footer in the report telling the user to restart OpenCode and `bun install`. Same model as `commands/sync-configs.md` overwriting itself today.
