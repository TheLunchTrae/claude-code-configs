...
- **`experimental.chat.system.transform` hook** — on every message, appends an `Instincts (<project>) — follow when the "when" fires:` block to `output.system`, one `<trigger>: <note>` line per entry. Slugs are intentionally dropped from the injected block (the LLM doesn't need the primary key; it only needs the rule). A hard `INJECT_MAX_CHARS = 2000` cap (≈500 tokens at 4 chars/token) truncates with a `…` footer so rule-count growth can't silently bloat every prompt. When `instincts.txt` is missing or empty, nothing is pushed.
- **Custom tools** — registers `memory_list`, `memory_write`, and `memory_delete`. No `memory_read` — entries are one short line each, returned in full by `memory_list`. Storage is two pipe-delimited files per project: `~/.opencode-artifacts/<project>/memory/instincts.txt` (columns `slug|trigger|note`) and `~/.opencode-artifacts/<project>/memory/facts.txt` (columns `slug|domain|note`). `memory_write` requires `kind: "instinct" | "fact"`; instincts require `trigger`, facts require `domain`. Values may not contain `|`, `\n`, or `\r`. Slugs are validated against `^[a-z0-9][a-z0-9-]{0,63}$` and are unique per project across both kinds — a cross-kind collision errors with a "delete first" hint. `memory_list` defaults to `kind: "instincts"` and returns raw lines; `kind: "facts"` supports a `domain` filter; both support exact-match `slug` lookup. `memory_delete` requires `confirm: true` and accepts `slug` / `kind` / `domain` / `project` for scoping.
- **Atomic writes** — `atomicReplace` writes to `<path>.tmp-<pid>-<ts>` then `rename`s onto the target so a crash mid-write can't leave a half-written file in place. Files are always sorted lex-by-slug on write so diffs (if ever inspected) stay stable.
- **No TTL prune** — memory entries are durable by design. Pruning is manual via `memory_delete`.
- **Token-cost discipline** — positional pipe-delimited columns with no field names on disk; instincts auto-injected into the system prompt instead of surfaced via a tool-call round-trip; facts tool-gated so they cost nothing per session when irrelevant; `INJECT_MAX_CHARS` cap on the injected block. Tool descriptions carry the when-to-write / when-not-to-write guidance so it is visible at tool-call time. Note hard-cap remains 240 chars.
