...
- The version is stored per project (`memory_write` defaults to `scope: "project"`, and the project name is resolved from the git remote → repo dir → cwd, in that order). Run `/sync-configs` from the same working directory each time so the same project is resolved on read and write.
