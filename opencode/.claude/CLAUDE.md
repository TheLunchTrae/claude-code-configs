...
### /manage-storage — `opencode/commands/manage-storage.md`
...
Agent-driven sync orchestrated by the slash command, backed by a thin four-tool plugin (`.opencode/plugins/sync-configs.ts`). The plugin provides metadata + helpers; the agent walks the sync loop using its built-in `Read`, `Write`, and `Bash` tools so each tool result stays small and so path resolution can honor whatever configs-root the user's project-level `AGENTS.md` designates. The plugin lives under `.opencode/plugins/` so it auto-discovers as a project-local plugin only when OpenCode loads this repo's `.opencode/` config dir, not in unrelated workspaces.

Procedure: (1) call `sync_configs_get_manifest` → returns `{version, paths, deleted, last_local_version, short_circuit}`; on `short_circuit` the command exits with `Already up to date (version <N>).`. (2) Determine the configs root from cwd or `AGENTS.md`. (3) Split the flattened sync list into batches of ~8-10 paths and dispatch one subagent per batch via the built-in `Task` tool, running all batches in parallel. Each subagent walks its assigned paths: call `sync_configs_fetch_file({path})` for upstream, call `sync_configs_classify({path, local_path, remote})` for the verdict (the plugin reads the local file itself), `Write` upstream for `updated`, no-op for `unchanged`, and **return needs-user-decision entries to the primary without acting** (so user-facing choices stay centralised). (4) Primary handles `needs-user-decision` cases sequentially: verbatim quote of `local_only_lines`, recommendation **drop** by default; choices drop / preserve / custom / skip. (5) Apply deletions from `manifest.deleted` only when `last_local_version` was defined (first-run safety). (6) Call `sync_configs_record_version({version})` only when `failed[]` and `unresolved[]` are both empty — partial-failure runs leave the version state alone so the next `/sync-configs` retries. (7) Print the structured report (Updated / Unchanged / Preserved / Deleted / Failed / Unresolved with footers for non-advanced version, first-run skipped deletions, and plugin self-update).

`opencode.jsonc` is hard-coded as always-defer inside `sync_configs_classify` so the slash command never has to special-case it. First-time setup (curl the plugin and slash command files from upstream) is documented at the OpenCode config root `README.md`.

OpenCode-exclusive because: it writes directly to `~/.claude/` config files and is designed for syncing the global OpenCode config setup, not for use within individual projects. Also depends on the OC-only `sync-configs` plugin.
...
- `<project>` is `basename(PluginInput.project.worktree)` — OpenCode resolves the worktree itself (walks up to a VCS root, falls back to the launch dir for non-git workdirs), so plugins never spawn git. Note: this means the storage key is the local clone-directory name, not the GitHub repo name; cloning a repo into a renamed dir gives it a different storage scope.
...
Authored OpenCode plugins live in `opencode/plugins/` (auto-discovered globally — every OpenCode session gets them) or under `opencode/.opencode/plugins/` (auto-discovered as project-local — only loaded in sessions that have this `.opencode/` config dir on the path). Both directories are scanned by OpenCode's loader (`{plugin,plugins}/*.{ts,js}` glob, applied to each config-bearing dir). Choose `.opencode/plugins/` for project-scoped plugins like `sync-configs.ts` that should only affect this repo. Choose `plugins/` for cross-project utilities.

Each plugin tree has its own `tsconfig.json` co-located one level above its `plugins/` directory: `opencode/tsconfig.json` covers `opencode/plugins/**/*.ts`, and `opencode/.opencode/tsconfig.json` covers `opencode/.opencode/plugins/**/*.ts`. The two tsconfigs are otherwise identical and exist purely for editor IntelliSense — OpenCode bundles its own TS support at runtime and does not consult either file. `opencode/package.json` lets contributors run `bun install` at the OC config root for editor type-checking; OpenCode itself auto-installs `@opencode-ai/plugin` per config dir at startup, so users on a clean install never need to run `bun install` manually.

`opencode/package.json` is intentionally excluded from the `/sync-configs` manifest — OpenCode generates one automatically at runtime on a user's install, so syncing the committed copy would clobber theirs. It stays in this repo only to support local plugin-authoring IDE type-checking. Both `tsconfig.json` files are synced because OpenCode does not auto-generate them.
...
- **Shared helpers** — `ARTIFACT_ROOT`, `MEMORY_ROOT`, `removeEmptyDir`, `DeleteResult`, `deleteFile`, and `formatErr` live in `plugins/lib/project.ts` and are imported by both `memory.ts` and `artifacts.ts`. Edit there, not in copies. The current project name is computed inline in each plugin as `basename(project.worktree)` — no helper needed.
...
### `.opencode/plugins/sync-configs.ts` — SyncConfigsPlugin

- **Project-local placement** — lives under `.opencode/plugins/` (not `plugins/`) so it auto-discovers only when OpenCode loads this repo's `.opencode/` config dir. Keeps the sync-configs tools out of unrelated workspaces.
- **Custom tools** — registers four small helpers used by the `/sync-configs` slash command: `sync_configs_get_manifest`, `sync_configs_fetch_file`, `sync_configs_classify`, `sync_configs_record_version`. The slash command does the orchestration (per-path Read/Write loop, drift surfacing, version persist gating) using these helpers plus the agent's built-in `Read`/`Write`/`Bash`. Each tool description starts with "Internal — invoked only by the /sync-configs slash command. Do not call autonomously" — the agent should never reach for them during normal coding sessions. Tool results are intentionally tiny (no file bodies in the manifest tool, single-file bodies only in `fetch_file`, line metadata only in `classify`) so the agent never hits the "output very large, let me read the full output file" truncation behavior.
- **Path resolution belongs to the slash command, not the plugin.** None of the tools take a configs-root or any local path — `Read`/`Write` are the agent's job, performed at whatever configs-root the user's project-level `AGENTS.md` (or cwd) designates. This means the plugin works correctly even when OpenCode runs at a directory above the configs subdir.
- **`sync_configs_get_manifest`** — read-only, no args. Fetches `raw.githubusercontent.com/thelunchtrae/claude-code-configs/main/opencode/.opencode/sync-configs-manifest.json`, `JSON.parse`s it, reads the local last-synced version state. Returns `{version, paths, deleted, last_local_version, short_circuit}`. `short_circuit: true` when manifest version equals local version; the slash command exits early. `paths` is an object whose values are string arrays per category — caller flattens with `Object.values(paths).flat()`.
- **`sync_configs_fetch_file({path})`** — read-only. Fetches one upstream file by manifest-relative path. Returns `{ok: true, body}` or `{ok: false, reason}`. Rejects empty bodies and HTML 404 sentinels. Used in the slash command's per-path loop.
- **`sync_configs_classify({path, local_path, remote})`** — reads `local_path` itself (caller passes a full path resolved against the configs root) and compares against `remote`. Returns `{status: "unchanged"}`, `{status: "updated"}` (covers local-missing AND the case where every non-empty local line also appears in remote, so applying upstream loses nothing the user added), or `{status: "needs-user-decision", local_only_lines, reason}`. `opencode.jsonc` is hard-coded in the always-defer set and returns `needs-user-decision` regardless of subset analysis (the `permission` block requires manual judgment per `opencode/.claude/CLAUDE.md`). Reading the local file inside the plugin (instead of requiring the caller to pre-read and pass the body) keeps file bodies out of the LLM round-trip.
- **`sync_configs_record_version({version})`** — mutating. Atomically writes `~/.opencode-data/sync-configs/<project>/version` (project = `basename(PluginInput.project.worktree)`). The slash command calls this only after a fully successful sync — no failures, no unresolved needs-user-decision paths. Partial-failure runs leave the state untouched so the next `/sync-configs` re-enters and retries.
- **Self-contained** — does not import from `plugins/lib/project.ts`. The two helpers it uses (`formatErr`, `SYNC_CONFIGS_ROOT`) are inlined since the file is in a different plugin tree (`.opencode/plugins/` vs `plugins/`) and a cross-tree import would be ugly. `lib/project.ts` stays scoped to `artifacts.ts` + `memory.ts`.
- **Storage** — `~/.opencode-data/sync-configs/<project>/version` contains just the integer (no schema, no format). Sibling subtree to `artifacts/` and `memory/`. Note: `<project>` is keyed on the OpenCode worktree, not the configs subdir — so a user running OpenCode above the configs gets a stable project-scope key from the parent directory's basename. Removing the version file forces the next run to re-sync every tracked file from scratch, but entries in the manifest's `deleted` array will be skipped on that run since the plugin cannot prove those files came from a previous sync.
- **First-run bootstrap** — `/sync-configs` is plugin-only; there is no inline curl fallback. Brand-new users (not cloning the repo) bootstrap the plugin via the shell snippet documented under "First-time setup" in `opencode/README.md`, which curls `.opencode/plugins/sync-configs.ts`, `.opencode/commands/sync-configs.md`, and `.opencode/tsconfig.json` from upstream. OpenCode auto-installs `@opencode-ai/plugin` for each config dir at startup (see `/tmp/oc/packages/opencode/src/config/config.ts:551-572` — `npmSvc.install` runs per dir), so no manual `bun install` is needed. After restart, `/sync-configs` works.
- **Plugin self-update** — `.opencode/plugins/sync-configs.ts` is in the manifest. Plugins load at session start into closed-over JS objects in memory; overwriting the `.ts` file mid-run does not affect the running tool, only the next session. The slash command's report appends a restart reminder when its own file was among the updated paths.
- **Toast on entry** — `get_manifest` and `record_version` fire a best-effort `client.tui.showToast` at entry (`"fetching manifest"` / `"recording version <N>"`) so the user sees the sync make progress even though the agent is doing most of the work. `fetch_file` and `classify` do not toast (they fire many times per sync — would be noisy).
- **Defensive error handling** — every tool `execute()` body is wrapped in `try`/`catch`. Failures return a JSON-encoded `{ error: "<tool> failed: <message>" }` (or for fetch/record, the natural shape with an `ok: false` + `reason`) string. Errors are also logged via `client.app.log` at `error` level with `console.error` fallback.
...
1. Create the TS file under `opencode/plugins/` for cross-project plugins, or `opencode/.opencode/plugins/` for plugins that should only load in this repo's project (see `sync-configs.ts` for the project-local pattern).
2. If it needs a new npm dep, add it to `opencode/package.json` and note the reason in the plugin's opening comment.
3. Add the plugin to the `## Registry` above and to `paths.plugins` in `opencode/.opencode/sync-configs-manifest.json` so it propagates on `/sync-configs`.
4. For `plugins/` placements, add a one-line row to the inventory table in `opencode/plugins/README.md`. (`.opencode/plugins/` placements are documented in `opencode/.opencode/README.md`'s Files table.)
5. No change to `opencode.jsonc` is needed — plugins are auto-discovered from `{plugin,plugins}/*.{ts,js}` relative to every config-bearing directory OpenCode loads (including the project-local `.opencode/`).
