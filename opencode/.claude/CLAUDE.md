...
Implemented as a hybrid plugin tool + thin slash command: `.opencode/plugins/sync-configs.ts` does the mechanical work (manifest fetch, version compare, parallel path fetch, classification, atomic version persist), and the slash command is a wrapper that calls `sync_configs_plan` → surfaces any `needs-user-decision` cases to the user (with the `local_only_lines` quoted verbatim and the recommendation defaulting to **drop**) → collects preserve / drop / custom choices → calls `sync_configs_apply` with the decisions → prints the structured report. The plugin lives under `.opencode/plugins/` so it auto-discovers as a project-local plugin only when OpenCode loads this repo's `.opencode/` config dir, not in unrelated workspaces. The plan tool short-circuits with `Already up to date (version <N>).` when the local version (read from `~/.opencode-data/sync-configs/<project>/version`) matches the manifest. On version mismatch the plan classifies each path as `unchanged`, `updated` (write remote verbatim — covers local-missing and the case where every non-empty local line also appears in the remote, so applying upstream loses nothing the user added), `needs-user-decision` (local has drift lines not present upstream, or path is `opencode.jsonc` which always defers), or `failed` (HTTP non-2xx, empty body, HTML 404 sentinel). The apply tool writes files atomically, deletes entries from the manifest's `deleted` array (returned empty on first-run for safety), and persists the version only when nothing failed and every needs-user-decision case was resolved — partial-failure runs leave the version untouched so the next `/sync-configs` re-enters and retries. When `.opencode/plugins/sync-configs.ts` was among the updated paths, the report appends a restart reminder (no manual `bun install` — OpenCode auto-installs plugin deps at startup). First-time setup (curl the plugin file from upstream) is documented in `opencode/.opencode/README.md`.
...
- **Custom tools** — registers `sync_configs_plan` and `sync_configs_apply`. These are the implementation of `/sync-configs`; the slash command is a thin wrapper that calls plan, surfaces `needs-user-decision` cases conversationally, then calls apply with collected decisions. Tool descriptions explicitly say "Internal — invoked only by the /sync-configs slash command. Do not call autonomously" — the agent should never reach for these during normal coding sessions.
- **`sync_configs_plan`** — read-only. Fetches the manifest from `raw.githubusercontent.com/thelunchtrae/claude-code-configs/main/opencode/.opencode/sync-configs-manifest.json`, `JSON.parse`s it, flattens `paths` (an object whose values are string arrays per category) into a single sync list, reads the local last-synced version, and fetches every tracked path in parallel via global `fetch()` (Node 18+; no curl). Each path is classified: `unchanged` (identical to upstream — metadata only), `updated` (local missing, or every non-empty local line also appears in upstream so applying remote loses nothing the user added — **metadata only, no body**), `needs-user-decision` (local has lines not present upstream — drift; includes `remote`, `local`, `reason`, `local_only_lines` so the LLM can construct a "custom" merge if the user picks that action), or `failed` (HTTP non-2xx, empty body, or HTML 404 sentinel). `opencode.jsonc` is hard-coded as always-defer when content differs (the `permission` block requires manual judgment per `opencode/.claude/CLAUDE.md`). Returns `short_circuit` when manifest version equals local version. Writes nothing. Plan output is intentionally body-free for `updated` paths so cold-start runs (where every path is `updated`) don't blow the LLM tool-result size budget.
- **`sync_configs_apply`** — mutating. Takes `manifest_version`, `paths_to_update` (plain `string[]` of paths classified `updated`), `decisions` (one per resolved `needs-user-decision` path: `{path, action: "preserve"}`, `{path, action: "drop"}`, or `{path, action: "custom", content}`), `unresolved` (skipped paths), `delete_paths`, and `failed` from the plan. Re-fetches the manifest first and aborts if the version no longer matches `manifest_version` (defensive — closes the plan→apply race so a user reviewing drift can't accidentally apply a newer upstream). Re-fetches every path in `paths_to_update` and every `drop` decision in parallel via `Promise.all` and writes the upstream bodies atomically (temp file + rename via `atomicReplace`). `preserve` is a pure no-op (the local file is already what the user wants). `custom` writes the supplied content verbatim. Deletes `delete_paths` only when the plan returned them (first-run safety: plan returns empty `delete_paths` when no prior version was found). Persists `manifest_version` to `~/.opencode-data/sync-configs/<project>/version` only when nothing failed and `unresolved` is empty — partial-failure runs leave the file untouched so the next `/sync-configs` re-enters and retries.
...
- **First-run bootstrap** — `/sync-configs` is plugin-only; there is no inline curl fallback. Brand-new users (not cloning the repo) bootstrap the plugin via the shell snippet documented under "First-time setup" in `opencode/.opencode/README.md`, which curls `.opencode/plugins/sync-configs.ts` and `tsconfig.json` from upstream. OpenCode auto-installs `@opencode-ai/plugin` for each config dir at startup (see `/tmp/oc/packages/opencode/src/config/config.ts:551-572` — `npmSvc.install` runs per dir), so no manual `bun install` is needed. After restart, `/sync-configs` works.
- **Plugin self-update** — `.opencode/plugins/sync-configs.ts` is in the manifest. Plugins load at session start into closed-over JS objects in memory; overwriting the `.ts` file mid-run does not affect the running tool, only the next session. The apply tool detects when its own file was among the updated paths and surfaces a footer in the report telling the user to restart OpenCode. Same model as `commands/sync-configs.md` overwriting itself today.
- **Toast on entry** — both tools fire a best-effort `client.tui.showToast` at entry (`"planning sync"` / `"applying sync"`) so accidental autonomous invocation is observable outside the tool-result panel. Borrowed from `block-secrets.ts`.
- **Defensive error handling** — every tool `execute()` body is wrapped in `try`/`catch`. Failures return a JSON-encoded `{ error: "<tool> failed: <message>" }` string (the apply/plan tools return JSON throughout for parseability, not the plain string convention used by artifacts/memory). Errors are also logged via `client.app.log` at `error` level with `console.error` fallback.
